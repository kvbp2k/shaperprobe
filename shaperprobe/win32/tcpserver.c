...
int prober_bind_port(int port)
{
	...
	sock = (int)socket(PF_INET, SOCK_DGRAM/*SOCK_STREAM*//*SOCK_RAW*/, IPPROTO_UDP);
	if(sock == -1)
	{
...
int create_server()
{
	...
	if ( (list_s = (int)socket(AF_INET, SOCK_STREAM, 0)) < 0 ) 
	{
	...
	optval = 1;
	setsockopt(list_s, SOL_SOCKET, SO_REUSEADDR, (char *)&optval, sizeof(optval));
...
int handle_clients(int list_s, int udpsock0)
{
	...
	int conn_s;
	if ( (conn_s = (int)accept(list_s, NULL, NULL) ) < 0 ) {
		fprintf(stderr, "SERV: Error calling accept()\n");
		return -1;
	}
	return conn_s;
...
__inline double timeval_diff(struct timeval x, struct timeval y)
{
...
#else
double capacityEstimation(int tcpsock, int udpsock0, struct sockaddr_in *from, FILE *fp, void *edit, int (*estatus)(char *, void *))
{
	...
	maxfd = (udpsock0 > tcpsock) ? udpsock0+1 : tcpsock+1;
	...
	while(1)
	{
		...
		sprintf(str, "Download packet train %d: %.0f Kbps", niters, tcap); estatus(str, edit);
		pcapack.capacity = htonl((unsigned long)cap);
		pcapack.finalflag = 0;
		pcapack.trainlength = htonl(TRAIN_LENGTH);
	...
	estatus("", edit); ncaps = 0; pcaps = traincaps;
/*	for(ret1=0; ret1<10*NITERATIONS; ret1++)
	...
	//XXX: hack(!) for Win32+802.11 downstream cases
	//if(caps[0] < mediancap*0.7 && (mediancap > 15000 && mediancap < 40000))
	//	mediancap = caps[0];

	pcapack.finalflag = htonl(1);
	pcapack.capacity = htonl((long)mediancap);
	ret = writewrapper(tcpsock, (char *)&pcapack, 
			sizeof(struct _capestack));
	if(ret == -1)
	{
